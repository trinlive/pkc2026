...
const uploadDir = path.join(__dirname, '../public/uploads/news_activity');
const tempUploadDir = path.join(__dirname, '../public/uploads/news_activity/temp');
...
module.exports = (req, res, next) => {
    upload.fields([
        ...
        try {
            ...
            // Process image upload with naming: activity_$timestamp
            if (imageFile) {
                const tempPath = imageFile.path;
                const timestamp = Date.now();
                const ext = path.extname(imageFile.filename).toLowerCase();
                const baseFilename = `activity_${timestamp}`;
                const finalPath = path.join(uploadDir, `${baseFilename}.webp`);
                ...
                await sharp(tempPath)
                    ...
                    .webp({ quality: 75 })
                    .toFile(path.join(uploadDir, `${baseFilename}-thumb.webp`));

                fs.unlinkSync(tempPath);
                req.uploadedImage = `/uploads/news_activity/${baseFilename}.webp`;
                req.uploadedThumb = `/uploads/news_activity/${baseFilename}-thumb.webp`;
            }

            // Process attachment upload with naming: _$timestamp
            if (attachmentFile) {
                const ext = path.extname(attachmentFile.filename).toLowerCase();
                const timestamp = Date.now();
                const attachmentName = `_${timestamp}${ext}`;
                ...
                req.uploadedAttachment = `/uploads/news_activity/${attachmentName}`;
            }
